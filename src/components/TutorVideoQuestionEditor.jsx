...
export default function TutorVideoQuestionEditor({ video, token, onClose }) {
  ...
  return <div className="fixed inset-0 z-[70] overflow-y-auto bg-black/60 p-4 backdrop-blur-sm">
    <div className="mx-auto my-0 sm:my-4 max-w-5xl rounded-2xl bg-white shadow-2xl">
      <div className="flex items-start sm:items-center justify-between gap-3 px-4 sm:px-6 py-4 sm:py-5 shadow-[0_1px_12px_rgba(15,23,42,0.06)]"><div><h2 className="text-lg sm:text-xl font-bold text-neutral-900">จัดการคำถามระหว่างวิดีโอ</h2><p className="mt-1 text-sm text-neutral-500">กำหนดจุดคำถามและติดตามความเข้าใจของผู้เรียนในคลิป “{video.VideoTitle}”</p></div><button onClick={onClose} className="shrink-0 rounded-xl border-0 p-2 text-neutral-400 outline-none hover:bg-orange-50 hover:text-orange-500"><X /></button></div>
