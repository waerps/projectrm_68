...
export default function ChatWidget() {
  ...
  if (!isOpen && !isFullscreen) {
    return (
      <button
        ...
        className="
          fixed bottom-6 right-10 z-50 h-16 w-16 rounded-full
  ...
  // ถ้า fullscreen อยู่ ให้ widget ไม่ต้องโชว์
  ...
  return (
    <>
      ...
      <div
          className="
            fixed z-50 bg-white rounded-3xl shadow-2xl flex flex-col
            right-4 bottom-4
            w-[calc(100vw-2rem)] max-w-[420px]
            h-[72vh] max-h-[640px]
            md:right-6 md:bottom-6 md:h-[470px]
          "
      >
        {/* Header */}
        <div className="px-5 py-4 border-b border-gray-100 rounded-t-3xl flex items-center justify-between ">
          <div className="flex items-center gap-3">
            <div className=" rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center font-bold">
                <img
                  src="/chatbot.png"
                  alt="Chatbot"
                  className="h-9 w-9 select-none pointer-events-none"
                  draggable="false"
                />
            </div>
            <div>
